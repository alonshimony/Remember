...
The database and configured model provider can read permitted text. This is not end-to-end encryption. All spaces are owner-only; space membership is also enforced on composite relationships. Authenticated browser access cannot mutate revisions, capture rows or jobs directly. Clerk authenticates accounts. A verified Clerk identity must match an invited owner before the server provisions or opens a workspace. Registering with Clerk does not by itself grant database access. Authenticated queries use a transaction-local PostgreSQL role and owner UUID; the browser never supplies a trusted owner identity.
...
| ---------------------------- | ------------------------------------------------------------------------------------------------ | ----------------------------------------------------- |
| Owner B guesses owner A's ID | RLS, composite owner/space foreign keys, API user validation                                     | Live Clerk/Neon verification                          |
...
| Forged worker call           | CRON_SECRET bearer check; public Clerk key is insufficient                                       | Hosted gateway invocation                             |
...
Clerk session validation happens before owner-scoped SQL. Owner mapping and invitations are inaccessible to the authenticated SQL role. The app API permits a fixed set of tables/functions and parameterizes values; privileged worker/purge functions have no browser gateway. Direct private-file access runs under owner RLS. Browser requests use Clerk cookies and mutation routes reject foreign origins. The service worker does not cache API responses.

The database-owner connection is a server secret and is privileged; its disclosure compromises database isolation. Permanent deletion uses Clerk password verification; passwordless accounts must add a password before using this purge flow. Clerk-hosted scripts are permitted by CSP for sign-in; no application session bearer secret is copied to localStorage. The offline cache holds an owner hint only, never authority to access the server.
