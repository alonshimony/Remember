...
Two different protections are needed: provider database recovery and a portable owner archive containing attachment **bytes**. Supabase database backups do not include Storage object bytes; see the official reference in DECISIONS.md.
...
Interactive exports currently refuse accounts exceeding 1,000 rows in any exported table or 40 MB of attachments. They do not claim to be complete when gateway row limits truncate data. Larger accounts need the pending multipart implementation. In the meantime an administrator can use Supabase CLI database dumps plus a separately verified Storage copy through its S3-compatible interface; that is an operational backup and is not the app's portable ZIP format.

## Restore drill

1. Create an isolated Supabase project and apply migrations. Disable Cron, AI and outgoing push.
...
The automated restore test creates two independent PostgreSQL engines and checks database fidelity. It does not verify hosted Auth UUID provisioning, object-storage transport or disaster recovery of a real account.
