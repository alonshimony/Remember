...
The user's follow-up replaces the original Supabase/Render choices with **Neon + Clerk + Vercel**. Runtime Supabase packages and the Deno worker have been removed. Vercel builds run transactional migrations and invite `OWNER_EMAIL`; a verified Clerk sign-in provisions the UUID workspace. Database RLS remains in place. Private files now live in Neon (3 MB per file); interactive ZIPs are capped at 4 MB. The default Vercel Cron is daily for Hobby compatibility; timely reminders need a more frequent supported scheduler.
...
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| M0        | Next.js/React/TypeScript/Tailwind, Neon schema, invitation trigger, RLS, no-AI mode, generated row types, two-owner SQL tests                                                                                       | Local SQL passed; live Neon/Clerk still external                                             |
...
| M3        | Source-linked commitments/history, exact reminders, snooze/cancel/done, yearly birthday preparation with leap policy, durable delivery ledger, worker, protected Vercel Cron, opt-in generic push                   | SQL tests passed; deployment/transport/device verification open; digest below                |
...
- No live Neon or Clerk credentials were supplied. Docker is no longer required.
- Typecheck/lint/unit+SQL+protocol tests/build checks and production browser tests run; see TEST_RESULTS.md for exact counts and limitations.
- Last measured 10k timeline SQL p95: 26.48 ms on this Windows machine, excluding network and phone behavior.

## Migrations

001 core ownership/capture/revisions/jobs; 002 retrieval/budget; 003 scoped integrations/restore; 004 legacy Storage (excluded; replaced by db/files.sql); 005 fenced extraction/delivery; 006 corrections/moves/keyset timeline/security; 007 optional pgvector; 008 reminder guards/annual preparation; 009 lifecycle/rate limits/chunk jobs; 010 purge authorization; 011 bounded consistent archive snapshot.

All non-Storage/non-vector migrations are applied during the type-generation check. Integration tests exercise selected complete chains. **No migration was applied to a live Neon project.**
...
Neon DATABASE_URL, Clerk keys and OWNER_EMAIL in the environment; optional explicit AI model IDs/key; CRON_SECRET; VAPID values; authorized hosting deployment; independent backup destination; a real iPhone. Do not ask for passwords in chat.
...
3. Deploy an isolated Neon branch with Clerk development keys; verify real sessions/files/vector plus multi-session concurrency tests.
